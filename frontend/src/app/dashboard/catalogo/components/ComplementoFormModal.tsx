...
export function ComplementoFormModal({
  ...
  function handleOpenCreateOption(initialName: string) {
    setCreatingOptionInitialName(initialName)
  }

  async function handleOptionCreated(newOptionId?: number) {
    setCreatingOptionInitialName(null)
    if (!newOptionId) return

    try {
      const res = await fetch(`${API_URL}/options/${newOptionId}`, {
        headers: { Authorization: `Bearer ${getToken()}` },
      })
      if (!res.ok) return
      const optionData = await res.json() as OptionDto
      ...
      const newOpt: ComplementOptionDto = {
        ...
        option: {
          ...
          stockStatus: optionData.stockStatus,
      ...
      if (typeof complementId === 'number') {
        try {
          const result = await mutations.addOption(complementId, newOptionId)
          const realId = (result as unknown as { id?: number }).id ?? tempId
          setOptions((prev) =>
            prev.map((o) => (o.id === tempId ? { ...o, id: realId } : o)),
          )
        } catch (e) {
          toast.error(e instanceof Error ? e.message : 'Erro ao vincular opção')
          setOptions((prev) => prev.filter((o) => o.id !== tempId))
        }
      }
    } catch {
      // ignore fetch error — option was created but can be added later
    }
  }
  ...
  return (
    <>
        ...
        <DialogContent className="w-full max-h-[90vh] overflow-y-auto sm:max-w-2xl">
          ...
          {loading ? (
            ...
            <div className="space-y-6 py-2">
              ...
              {/* Seção 6: Opções */}
              <ComplementoOptionsSection
                complementId={typeof complementId === 'number' ? complementId : null}
                options={options}
                onChange={setOptions}
                onCreateNewOption={handleOpenCreateOption}
                disabled={submitting}
              />
      ...
      <OpcaoFormModal
        open={creatingOptionInitialName !== null}
        onClose={() => setCreatingOptionInitialName(null)}
        optionId="new"
        initialName={creatingOptionInitialName ?? ''}
        onSaved={handleOptionCreated}
      />
    </>
  )
}
