...
export class MailService {
  ...
  async sendWelcome(to: string, nome: string, slug: string) {
    ...
    const html = baseTemplate(
      ...
      'Acessar meu painel', 'https://cardapiopedeai.com.br/dashboard',
    );
  ...
  async sendTrialEnding(to: string, nome: string, daysLeft: number) {
    const html = baseTemplate(
      ...
      'Assinar agora', 'https://cardapiopedeai.com.br/dashboard/assinatura',
    );
    await this.send(to, `Atenção: trial expira em ${daysLeft} dias — ${BRAND}`, html);
  }

  async sendSubscriptionConfirmed(to: string, nome: string, plan: string, nextBilling: string) {
    const html = baseTemplate(
      ...
      'Acessar painel', 'https://cardapiopedeai.com.br/dashboard',
    );
  ...
  async sendSubscriptionCanceled(to: string, nome: string) {
    const html = baseTemplate(
      ...
      'Reativar assinatura', 'https://cardapiopedeai.com.br/dashboard/assinatura',
    );
  ...
  async sendPaymentFailed(to: string, nome: string) {
    const html = baseTemplate(
      ...
      'Atualizar pagamento', 'https://cardapiopedeai.com.br/dashboard/assinatura',
    );
    await this.send(to, `Falha no pagamento — ${BRAND}`, html);
  }

  async sendNpsRequest(to: string, nome: string, orderNumber: number, npsUrl: string) {
    const html = baseTemplate(
